...
void jacobi ( int m, int n, double alpha, double omega, double u[], double f[], 
  ...
  */
  ...
  // USE: u_old = ( double * ) _mm_malloc (m * n * sizeof(float), 64);
...
double *rhs_set ( int m, int n, double alpha ) {
  ...
  // USE: f = ( double * ) _mm_malloc (m * n * sizeof(double), 64);
  ...
  for ( j = 0; j < n; j++ )
  {
    ...
    // USE: #pragma omp parallel for    |   aligned(f: 64) private(i, x, y) shared(f)
    // about the same for the next 4 loop kernals but i dont understand why there is multiple????????
    for ( i = 0; i < m; i++ )
    {
      j = 0;
      y = ( double ) ( 2 * j - n + 1 ) / ( double ) ( n - 1 );
  ...
  // USE: #pragma omp parallel for  |   aligned(f: 64) private(i, j) shared(f) reduction(+:f_norm)
  for ( j = 0; j < n; j++ )
  {
    for ( i = 0; i < m; i++ )
    {
